...
export class Cloth {
    ...
    constructor(gridColumns: number = 2, gridRows: number = 2, coords: Vector = new Vector([100, 100, 0]), playerControlled1: boolean =true, playerControlled2: boolean = true, dynamic: boolean = true, gravityTrue: boolean = true, spacing: number = 100) {
        ...
        for (let i = 0; i < rows; i++) {
          for (let j = 0; j < columns; j++) {
            if (j === 0 ) {
                let anchour = new Anchour((new Vector([this.spacing + i * this.spacing + 0.01*this.spacing*j, this.spacing + j* this.spacing + 0.01*this.spacing*i, 0])).add(this.coords))
                this.physicsObjectArray[i*columns + j] = anchour
            } else {
                ...
                let particle = new PhysicsObject(circleShape, particlePM, playerControlled1, playerControlled2, dynamic, gravityTrue, 2);
        ...
        for (let i = 0; i < rows; i++) {
          for (let j = 0; j < columns; j++) {
                ...
                if (ni >= 0 && ni < rows && nj >= 0 && nj < columns) {
                    ...
                    let spring = new Spring(particle, neighbor, this.spacing*magnitude*.8, 0.01*magnitude);
    ...
    findOutsidePoints() {
        ...
        let points = this.physicsObjectArray.map((value: PhysicsObject) => 
            value.shape.coords.values.slice(0, 2)
