...
import { canvas, ctx, fps, gravity, pressedKeys } from '../globals';
...
let cloth1 = new Cloth(4,4,new Vector([100,100, 0]), true, true, true, true, .050*canvas.width + .05*canvas.height)
// let softBody2 = new SoftBody(2,3,new Vector([500,500, 0]), false, false, true, true)

cloth1.physicsObjectArray.map((value: PhysicsObject) => {
  physicsObjectArray.push(value)})
cloth1.springArray.map((value: Spring) => {
